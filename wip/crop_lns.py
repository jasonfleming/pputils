...
#
# Modified: Jul 15, 2017
# Fixed the error related to lines completely not touching the
# crop polygon to make the script crash. Discovered that this script
# does not produce desired results when the crop boundary is irregular.
# In these instances the script can output contour lines that cross,
# which is detrimental to TIN modeling. The strategy used to crop the
# lines is not appropriate, and the script does not fully work in all
# cases. But, it still might be useful, so I am keeping it.
#
...
# find the vertices that will be used to write the cropped lines
idx = list()

c = -1 # counter
for i in range(len(line_node_count)):
  for j in range(line_node_count[i]):
    c = c + 1
    if (line_node_count[i] > 1):
      idx.append(c)

# now we are ready to write the cropped lines
for i in range(len(idx)):
  fout.write(str(int(shapeid_lns_cr[idx[i]])) + ',' + str(x_lns_cr[idx[i]])
    + ',' +  str(y_lns_cr[idx[i]]) + ',' + str(z_lns_cr[idx[i]]) + '\n')  
