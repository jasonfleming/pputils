...
!
  ...
  real ( kind = 16 ), allocatable, dimension ( :, : ) :: node_xy
...
!
!    Input/output, real ( kind = 16 ) A(M,N), the array to be permuted.
!
  implicit none
...
!
      ...
      if ( ihave == 3 ) then
        rtop = 10.0D+00 * rtop + real ( ndig, kind = 16 )
...
!
  if ( jtop == 0 ) then
    ...
    if ( jbot == 1 ) then
      ...
      rexp = 10.0D+00 ** ( real ( jsgn * jtop, kind = 16 ) &
        / real ( jbot, kind = 16 ) )
    end if
  end if
